...
class bgpDataEngine(object):
    ...
    def __init__(self,configfile='./conf/bgpDataEngine.conf'):
        ...
        try:
            ...
            self.dbFlag=True
            self.logger.info('Test connection to MySQL server on '+self.config['MySQL']['serverIP']+":"+self.config['MySQL']['serverPort']+' successful.')
            self.db.close()
        except:
            self.logger.warn('DB connection not established. load2DB() may not work!.')

        self.bgpmoncollectors=['routeviews2','routeviews3','routeviews4','routeviews6','bgpmon']
        self.rvcollectors =['route-views2','route-views4','route-views3','route-views6','route-views.eqix','route-views.isc','route-views.kixp','route-views.jinx' \
                ,'route-views.linx','route-views.nwax','route-views.telxatl','route-views.wide','route-views.sydney','route-views.saopaulo','route-views.sg', \
    ...
    def __downloadFileWorker(self):
        while True:
            ...
            dowloadFileNameWithPath=self.dirpath+collector+'/'+year+'/'+month+'/'+day+'/'+collector+'.'+os.path.basename(url)
                ...
                with open(indexFile,'r') as readFile:
                        for line in readFile:
                            if year+month+day in line:
                                ...
                                vals2=vals[1].split('\">')
                                fileNames.append(str(tryCounter)+'|'+collector+'|'+year+'|'+month+'|'+day+'|'+url+'/'+vals2[0])
                os.remove(indexFile)
    ...
    def getMonth(self,datatype,year,month):
        ...
        self.getRange(datatype,start, end)
...
#                     print('Error in dropping indexes.')
            ...
            with closing( dblocal.cursor() ) as cur: 
                try:
                    query="SELECT group_concat(ID), count(*) as count from {0} GROUP BY MsgHash HAVING count > 1;".format(table)
                    cur.execute(query)
                    rowAll=cur.fetchone()
                    while rowAll:
                        row=rowAll[0]
        ...
        try:
            ...
            def localPushData(table,data): 
                try:
                    ...
                    with closing( dblocal.cursor() ) as cur: 
                        ...
                        query="insert ignore into {0} (id,BGPVersion,MsgTime,MsgType,PeerAS,PeerIP,PrefixOriginAS,PrefixIP,PrefixMask,ASPath,ASPathLength,ASPathLengthSimple,Origin,NextHop,LocalPref,Med,Community,AggregateID,AggregateIP,MsgHash)".format(table)
                        try:
            ...
            def localPopulatePeerDayTable(table):
                ...
                if isPresent:
                    ...
                    with closing( dblocal.cursor() ) as cur:
                        try:
                            query="UPDATE peer_day SET {0}=1 WHERE PeerIP=\'{1}\' and Day=\'{2}\';".format(Ttype,peer,day)
                            cur.execute(query)
                            dblocal.commit()
                        except:
                    ...
                    with closing( dblocal.cursor() ) as cur:
                        ...
                        try:
                            if Tabletype=='updates':
                                #print('Table type: '+Tabletype)
                                query="insert into peer_day (PeerIP,Day,Updates,Ribs) values (\'{0}\',\'{1}\',\'{2}\',\'{3}\');".format(peer,day,1,0)
                                #print('inserted for updates table')
                            else:
                                query="insert into peer_day (PeerIP,Day,Updates,Ribs) values (\'{0}\',\'{1}\',\'{2}\',\'{3}\');".format(peer,day,0,1)
            ...
            for line in lines.split("\n"):
                ...
                if line.startswith("BGP") or line.startswith('TABLE'):#Eliminate possibility of empty rows
                    pieces = line.split('|')
                    #print(pieces)
                    (BGPVersion,MsgTime,MsgType,PeerIP,PeerAS,PrefixCom,ASPath,Origin,NextHop,LocalPref,Med,Community,AggregateID,AggregateIP)=[""]*14
                    if len(pieces)==15:
                        (BGPVersion,MsgTime,MsgType,PeerIP,PeerAS,PrefixCom,ASPath,Origin,NextHop,LocalPref,Med,Community,AggregateID,AggregateIP,_) = pieces
                    else:
                    ...
                    #str(counter)+'|'+
                    strg=BGPVersion+'|'+MsgTime+'|'+MsgType+'|'+str(PeerAS)+'|'+str(PeerIP)+'|'+str(PrefixOriginAS)+'|'+str(PrefixIP)+'|'+str(PrefixMask)+'|'+ASPath+'|'+str(lenASPath)+'|'+str(lenASPathClean)+'|'+Origin+'|'+str(NextHop)+'|'+str(LocalPref)+'|'+str(Med)+'|'+Community+'|'+AggregateID+'|'+AggregateIP
